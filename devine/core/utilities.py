...
def rotate_log_file(log_path: Path, keep: int = 20) -> Path:
    ...
    if log_path.parent.exists():
        log_files = [
            x for x in log_path.parent.iterdir() if x.suffix == log_path.suffix
        ]
...
def sanitize_filename(filename: str, spacer: str = ".") -> str:
    ...
    filename = filename.replace("/", " & ").replace(
        ";", " & "
    )  # e.g. multi-episode filenames
    filename = re.sub(r"[:; ]", spacer, filename)  # structural chars to (spacer)
    filename = re.sub(r"[\\*!?¿,'\"“”()<>|$#]", "", filename)  # not filename safe chars
    filename = re.sub(
        rf"[{spacer}]{{2,}}", spacer, filename
    )  # remove extra neighbouring (spacer)s

    return filename


def is_close_match(
    language: Union[str, Language], languages: Sequence[Union[str, Language, None]]
) -> bool:
    ...
    if not languages:
        return False
    return (
        closest_match(language, list(map(str, languages)))[1] <= LANGUAGE_MAX_DISTANCE
    )
...
class DebugLogger:
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                ...
                is_always_sensitive = not has_prefix and any(
                    ...
                    for sensitive in ["password", "token", "secret", "auth", "cookie"]
                )
                ...
                should_redact = is_always_sensitive or (
                    is_key_field and not self.log_keys
                )
    ...
    def log_drm_operation(self, drm_type: str, operation: str, **kwargs):
        ...
        self.log(
            level="DEBUG",
            operation=f"drm_{operation}",
            message=f"{drm_type} {operation}",
            drm_type=drm_type,
            **kwargs,
        )
    ...
    def close(self):
        ...
        if self.file_handle:
            self.log(
                ...
                message="Debug logging session ended",
            )
...
def init_debug_logger(
    log_path: Optional[Path] = None, enabled: bool = False, log_keys: bool = False
):
    """
