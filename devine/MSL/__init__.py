...
from devine.MSL.gen_esn import (
    android_esn_generator,
    chrome_esn_generator,
    playready_esn_generator,
)
from devine.MSL.MSLKeys import MSLKeys
from devine.MSL.schemes import EntityAuthenticationSchemes  # noqa: F401
from devine.MSL.schemes import KeyExchangeSchemes
from devine.MSL.schemes.EntityAuthentication import EntityAuthentication
from devine.MSL.schemes.KeyExchangeRequest import KeyExchangeRequest
from devine.MSL.schemes.PlayReadyKeyExchangeScheme import PlayReady as PlayReadyScheme
...
class MSL:
    ...
    @classmethod
    def handshake(
        cls, scheme, session, endpoint, sender=None, cdm=None, msl_keys_path=None
    ):
        ...
        if msl_keys is not None:
            ...
            if not cdm:
                raise EnvironmentError("- No cached data and no CDM specified")

            if not msl_keys_path:
                raise EnvironmentError("- No cached data and no MSL key path specified")
            ...
            if scheme == KeyExchangeSchemes.Widevine:
                msl_keys.cdm_session = cdm.open(
                    pssh=b"\x0a\x7a\x00\x6c\x38\x2b", raw=True, offline=True
                )
            ...
            if scheme == KeyExchangeSchemes.Widevine:
                ...
                try:
                    ...
                    cls.log.info(
                        "Created PlayReady entity authentication without parameters"
                    )
            ...
            try:
                ...
                raise EnvironmentError(
                    f"- Key exchange failed, response data is unexpected: {e.response.text}"
                )
            ...
            if "errordata" in key_exchange:
                raise EnvironmentError(
                    "- Key exchange failed: "
                    + json.loads(base64.b64decode(key_exchange["errordata"]).decode())[
                        "errormsg"
                    ]
                )

            # parse the crypto keys
            key_response_data = json.JSONDecoder().decode(
                base64.b64decode(key_exchange["headerdata"]).decode("utf-8")
            )["keyresponsedata"]

            if key_response_data["scheme"] != str(scheme):
                raise EnvironmentError("- Key exchange scheme mismatch occurred")
            ...
            if scheme == KeyExchangeSchemes.Widevine:
                ...
                try:
                    ...
                    raise EnvironmentError(
                        f"- PlayReady key decryption failed: {str(e)}"
                    )
            else:
                cipher_rsa = PKCS1_OAEP.new(msl_keys.rsa)
                msl_keys.encryption = MSL.base64key_decode(
                ...
                (
                    datetime.utcfromtimestamp(
                        int(
                            json.JSONDecoder().decode(
                                base64.b64decode(
                                    msl_keys.mastertoken["tokendata"]
                                ).decode("utf-8")
                            )["expiration"]
                        )
                    )
    ...
    def get_widevine_key(cls, kid, keys, permissions):
        for key in keys:
            ...
            if key.type != "OPERATOR_SESSION":
                cls.log.warning(
                    f"Widevine Key Exchange: Wrong key type (not operator session) key {key}"
                )
                continue
            if not set(permissions) <= set(key.permissions):
                cls.log.warning(
                    f"Widevine Key Exchange: Incorrect permissions, key {key}, needed perms {permissions}"
                )
                continue
            return key.key
        return None
    ...
    def send_message(self, endpoint, params, application_data, userauthdata=None):
        ...
        res = self.session.post(
            url=endpoint, data=message, params=params
        )  # verify=True?
        ...
        if "errordata" in header:
            raise EnvironmentError(
                "- MSL response message contains an error: {}".format(
                    json.loads(
                        base64.b64decode(header["errordata"].encode("utf-8")).decode(
                            "utf-8"
                        )
                    )
                )
    ...
    def create_message(self, application_data, userauthdata=None):
        ...
        header = json.dumps(
            {
                "headerdata": base64.b64encode(headerdata.encode("utf-8")).decode(
                    "utf-8"
                ),
        ...
        return json.dumps(
            {
                ...
                "keyid": "{}_{}".format(
                    self.sender,
                    json.loads(
                        base64.b64decode(self.keys.mastertoken["tokendata"]).decode(
                            "utf-8"
                        )
                    )["sequencenumber"],
                ),
