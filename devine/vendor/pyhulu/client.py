...
from devine.vendor.pyhulu.device import Device
...
class HuluClient(object):
    ...
    def __init__(
        self, device_code, device_key, cookies, version=1, extra_playlist_params={}
    ):
    ...
    def decrypt_response(self, key, ciphertext):
        ...
        try:
            ...
            self.logger.error(
                "Tried decrypting with key %s", base64.b64encode(key).decode("utf8")
            )
    ...
    def get_session_key(self):
        ...
        config_dict = self.decrypt_response(self.device.device_key, ciphertext)

        derived_key_array = bytearray()
        for device_byte, server_byte in zip(
            self.device.device_key, bytes.fromhex(config_dict["key"])
        ):
